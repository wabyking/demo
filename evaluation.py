    ...
    def alignment_quality_driver(self, log_filename= "alignment_quality.log"):
        ...
        with open(log_filename, "w", encoding="utf-8") as f:
            for df in [df1,df2]:
                ...
                # print(df)
                # print(len(df))
                embeddings, known_index = self.get_embedding_in_a_year(df.w1, df.y1.tolist(), return_known_index=True)
...
class Timer():
    ...
    def __init__(self,start_year =None, save_path = None, scale = None):
        ...
        if scale is None :
            if save_path is not None and "coha"  in save_path:
                self.scale = 10
            else:
                self.scale = 1
        else:
...
def yao_test( model_path = "nyt_yao.txt.train-output"):

    years,real_years = year_mapping[model_path]
    types= ["word_mixed_fixed"] if torch.cuda.is_available() else ["word_mixed"]
    for time_type in types:  # "word_cos", , "word_cos"
        # for epoch in range(1, 10, 1):
        epoch = None
...
def ssd_test(model_path = "coha"):
    years = [(i - 1810) // 10 for i in range(1810, 2020, 10)]
    real_years = [str(i) + "s" for i in range(1810, 2020, 10)]
...
if __name__ == '__main__':
    # ssd_test("coha.txt.raw.token.decade-output")
    # yao_test()
